...
#if defined(NDEBUG)
# error "USDG cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "USDG Signed Message:\n";
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake() && !CheckProofOfStake(pindex->pprev, block.vtx[1], block.nBits, state, block.vtx[1].nTime ? block.vtx[1].nTime : block.nTime)) {
    ...
    // USDG also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // USDG also requires low S in sigs
